...
export async function syncCityData(city: CityConfig, targetDate: string) {
  ...
  try {
    ...
    const hours = Array.from(
      { length },
      (_, i) => finalBaseTime + i * resolution,
    );

    const hourlyReport: HourlyReportItem[] = hours.map((timestamp) => {
      // Find existing data for this slot if any
      const existing = existingRecords.find(
        (r: any) => Math.abs(r.timestamp - timestamp) < 60, // Close enough
      );

      // New API data for history
      const historyMatches = (historyData.observations || []).filter(
        (obs: any) => {
          const obsTime = obs.valid_time_gmt;
          return Math.abs(obsTime - timestamp) <= tolerance;
        },
      );

      const historyEntry =
        historyMatches.length > 0
          ? historyMatches.reduce((max: any, obs: any) =>
      ...
      let history = historyEntry
        ? {
            temp: historyEntry.temp,
            condition: historyEntry.wx_phrase || historyEntry.phrase_32char,
          }
      ...
      if (!history && Math.abs(nowGmt - timestamp) < 2700) {
        if (
          currentData?.validTimeUtc &&
          Math.abs(currentData.validTimeUtc - timestamp) < tolerance
        ) {
          console.log(
            `[Sync] Using Live Fallback for ${city.name} at ${timestamp}: ${currentData.temperature}°C`,
          );
          history = {
      ...
      let forecastIndex = hourlyForecast.validTimeUtc?.findIndex(
        (t: number) => Math.abs(t - timestamp) < tolerance,
      );

      // Fallback: If no exact/near match for 30-min slot, look for the preceding hourly forecast
      if (
        (forecastIndex === undefined || forecastIndex === -1) &&
        hourlyForecast.validTimeUtc
      ) {
      ...
      const aviationHistoryMatches = metarData?.filter((obs: any) => {
        ...
        const reportTimestamp = Math.floor(
          new Date(reportTimeStr).getTime() / 1000,
        );
        return Math.abs(reportTimestamp - timestamp) <= tolerance;
      });
    ...
    const recordsToUpsert = hourlyReport.map((item: HourlyReportItem) => ({
      ...
      temp_f_wu:
        item.wuHistory?.temp !== undefined ? toF(item.wuHistory.temp) : null,
      forecast_c_wu: item.wuForecast?.temp ?? null,
      forecast_f_wu:
        item.wuForecast?.temp !== undefined ? toF(item.wuForecast.temp) : null,
    ...
    for (const record of recordsToUpsert) {
      ...
      `,
        [
          record.city_name,
          record.station_id,
          record.timestamp_gmt,
          record.city_time,
          record.wib_time,
          record.temp_c_wu,
          record.temp_f_wu,
          record.forecast_c_wu,
          record.forecast_f_wu,
          record.history_c_aviation,
          record.history_f_aviation,
          record.condition_history_wu,
          record.condition_forecast_wu,
          JSON.stringify(record.forecast_history_wu),
          record.forecast_updated_at_wu,
          record.wu_exact_time,
          record.wu_synced_at,
    ...
    console.log(
      `[Sync] Successfully synced ${recordsToUpsert.length} records for ${city.name}.`,
    );
...
export async function getWeatherFromDb(city: CityConfig, targetDate: string) {
  try {
    ...
    const baseTime = Math.floor(
      (new Date(year, month, day).getTime() + offset) / 1000,
    );
    ...
    const hourlyReport = data.map((record: any) => ({
      ...
      wuForecast:
        record.forecast_c_wu !== null
          ...
          : null,
      aviationHistory:
        record.history_c_aviation !== null
          ? {
